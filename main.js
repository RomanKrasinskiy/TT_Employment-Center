...
document.getElementById('filtersForm').addEventListener('submit', function (event) {
  ...
  // Получаем текущий номер страницы для отображения
...
function applyFiltersAndDisplay(vacancies, pageNumber, filters) {
  ...
  if (filters.jobName || filters.salary || filters.showOnlyWithSalary) {
    filteredVacancies = vacancies.filter(vacancy => {
      ...
      const jobNameFilter = filters.jobName ? vacancy.jobName.toLowerCase().includes(filters.jobName.toLowerCase()) : true;
      // Применяем фильтр по размеру зарплаты
      const salaryFilter = filters.salary ? vacancy.salary >= filters.salary : true;
      // Применяем фильтр для показа только вакансий с зарплатой
      const showOnlyWithSalaryFilter = filters.showOnlyWithSalary ? vacancy.salary > 0 : true;
